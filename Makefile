...
tidy:
	if [ -x ~/bin/perltidy ]; then \
           ~/bin/perltidy ./bin/chronicle-ping ./bin/chronicle-entry-filter ./bin/chronicle-spam-test ./bin/chronicle-spooler ./bin/chronicle-rss-importer ./bin/chronicle ./cgi-bin/comments.cgi  $$(find ./lib -name '*.pm') \
	; fi
